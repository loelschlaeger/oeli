...
inline arma::vec rmvnorm(
  ...
  if (!arma::all(arma::vectorise(Sigma) == 0)) {
    arma::mat L = arma::chol(Sigma, "lower");
    arma::vec eps(p);
    for (int j = 0; j < p; ++j) eps(j) = norm_rand();
    draw += L * eps;
  }
  if (log) {
    for (int j = 0; j < p; ++j) draw(j) = std::exp(draw(j));
  }
  return draw;
}

// helpers of the multivariate normal distribution function
namespace detail {
...
inline double bvn_upper(double h, double k, double r) {
...
inline double bvn_upper_extended(double h, double k, double r) {
...
inline arma::mat const& halton_points(int draws, int dims) {
...
inline void mvtdst(
  int* n, int* nu, double* lower, double* upper, int* infin, double* corr,
  double* delta, int* maxpts, double* abseps, double* releps, double* error,
  double* value, int* inform, int* rnd
) {
  typedef void (*integrator)(
    int*, int*, double*, double*, int*, double*, double*, int*, double*,
    double*, double*, double*, int*, int*
  );
  static integrator fun = NULL;
  if (fun == NULL) {
    fun = (integrator) R_GetCCallable("mvtnorm", "C_mvtdst");
  }
  fun(
    n, nu, lower, upper, infin, corr, delta, maxpts, abseps, releps, error,
    value, inform, rnd
  );
}
...
inline double pmvnorm(
  ...
  lower_std.fill(detail::negative_infinity);
  if (lower.isNotNull()) {
  ...
  if (p == 2) {
    ...
    double value =
      detail::bvn_upper_extended(lower_std[0], lower_std[1], r) -
      detail::bvn_upper_extended(upper_std[0], lower_std[1], r) -
      detail::bvn_upper_extended(lower_std[0], upper_std[1], r) +
      detail::bvn_upper_extended(upper_std[0], upper_std[1], r);
    return std::min(std::max(value, 0.0), 1.0);
  }
  if (p == 3) {
    double value = detail::tvn_rectangle(lower_std, upper_std, corr);
    if (!ISNAN(value)) return value;
  }
  if (method == "ghk") return detail::ghk(lower_std, upper_std, corr, draws);
  return detail::genz(lower_std, upper_std, corr, abseps);
}
